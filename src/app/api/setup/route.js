...
export async function POST(req) {
  ...
  try {
    ...
    // 1. Seed Machines (batched)
    if (machinesToSeed.length > 0) {
      const values = [];
      const params = [];
      machinesToSeed.forEach((m, i) => {
        const offset = i * 5;
        values.push(`($${offset+1}, $${offset+2}, $${offset+3}, $${offset+4}, $${offset+5})`);
        params.push(m.id, m.name, m.location, m.status, JSON.stringify(m.thresholds));
      });
      await client.query(
    ...
    // 2. Seed Inventory (batched)
    if (inventoryToSeed.length > 0) {
      const values = [];
      const params = [];
      inventoryToSeed.forEach((inv, i) => {
        const offset = i * 6;
    ...
    // 3. Seed Nodes (batched)
    if (supplierNodesToSeed.length > 0) {
      const values = [];
      const params = [];
      supplierNodesToSeed.forEach((node, i) => {
        const offset = i * 5;
        values.push(`($${offset+1}, $${offset+2}, $${offset+3}, $${offset+4}, $${offset+5})`);
        params.push(...node);
      });
    ...
    // 4. Seed Edges (batched)
    if (supplierEdgesToSeed.length > 0) {
      const values = [];
      const params = [];
      supplierEdgesToSeed.forEach((edge, i) => {
        const offset = i * 5;
        values.push(`($${offset+1}, $${offset+2}, $${offset+3}, $${offset+4}, $${offset+5})`);
        params.push(...edge);
      });
    ...
    // 5. Generate baseline telemetry for all machines (batched)
    const now = new Date();
